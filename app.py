...
@app.route('/painel/painel13')
@login_required
def painel13():
    """Página do Painel 13 - Prescrições de Nutrição"""
    usuario_id = session.get('usuario_id')
    is_admin = session.get('is_admin', False)

    if not is_admin:
        if not verificar_permissao_painel(usuario_id, 'painel13'):
            app.logger.warning(f'Acesso negado ao painel13: {session.get("usuario")}')
            return send_from_directory('frontend', 'acesso-negado.html')

    return send_from_directory('paineis/painel13', 'index.html')


@app.route('/api/paineis/painel13/nutricao', methods=['GET'])
@login_required
def api_painel13_nutricao():
    """
    Retorna dados das prescrições de nutrição
    Query params:
    - setor: Filtra por setor específico (ex: 'UTI Adulto')
    """
    usuario_id = session.get('usuario_id')
    is_admin = session.get('is_admin', False)
    ...
    try:
        ...
        app.logger.error(f'Erro ao buscar dados do painel13: {e}', exc_info=True)
        if conn:
            conn.close()
        return jsonify({
            'success': False,
...
@app.route('/api/paineis/painel13/setores', methods=['GET'])
@login_required
def api_painel13_setores():
    ...
    try:
        ...
        app.logger.error(f'Erro ao buscar setores do painel13: {e}', exc_info=True)
        if conn:
            conn.close()
        return jsonify({
            'success': False,
...
@app.route('/api/paineis/painel13/stats', methods=['GET'])
@login_required
def api_painel13_stats():
    ...
    try:
        ...
        app.logger.error(f'Erro ao buscar estatísticas do painel13: {e}', exc_info=True)
        if conn:
            conn.close()
        return jsonify({
            'success': False,
