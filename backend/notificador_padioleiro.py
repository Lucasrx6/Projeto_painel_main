"""
========================================
WORKER - NOTIFICADOR DE MOVIMENTAÇÕES DO PADIOLEIRO
========================================

Envia relatório de movimentações do padioleiro por email em horários configuráveis.

Configuração via .env:
  NOTIF_PADIOLEIRO_EMAILS    - destinatários separados por vírgula
  NOTIF_PADIOLEIRO_HORARIOS  - horários fixos separados por vírgula, ex: 06:00,18:00
  NOTIF_PADIOLEIRO_AUTO      - true/false (padrão: true)

O email contém:
  - Corpo HTML com resumo geral do dia
  - Anexo Excel com 6-7 abas: Resumo, Por Padioleiro, Por Setor,
    Distribuição por Hora, Tendência 7 Dias, Todos os Chamados, Cancelamentos
"""
...
def gerar_html(resumo, por_padioleiro, por_setor, cancelados, now):
    ...
    if not linhas_pad:
        linhas_pad = '<tr><td colspan="8" style="padding:16px;text-align:center;color:#aaa;">Nenhum chamado no período</td></tr>'
    ...
    if not linhas_set:
        linhas_set = '<tr><td colspan="5" style="padding:16px;text-align:center;color:#aaa;">Nenhum chamado no período</td></tr>'
...
<div style="max-width:720px;margin:24px auto;">
  ...
  <div style="background:#fff;padding:24px 32px;">
    ...
    <div style="margin-top:16px;background:#f8f9fa;border-radius:8px;padding:14px 20px;">
      <span style="font-size:13px;color:#444;">⏱ <strong>T.médio aceite:</strong> {_fmt(resumo.get('media_aceite_min'), ' min')}</span>
      <span style="font-size:13px;color:#444;margin-left:32px;">⏱ <strong>T.médio total:</strong> {_fmt(resumo.get('media_total_min'), ' min')}</span>
    </div>
  </div>
...
def executar_envio():
    ...
    if not cfg['destinatarios']:
        logger.warning("Nenhum destinatário em NOTIF_PADIOLEIRO_EMAILS. Pulando.")
        return
...
def main():
    logger.info("=" * 50)
    logger.info("Notificador de Movimentações Padioleiro — Iniciando")
    logger.info("=" * 50)

    cfg = _cfg()
    logger.info("Destinatários : %s", cfg['destinatarios'] or '(nenhum)')
    logger.info("Horários      : %s", cfg['horarios'] or '(não configurado)')
...
def start_in_background():
    ...
    def _run():
        try:
            cfg = _cfg()
            logger.info(
                '[notificador_padioleiro] Thread daemon iniciada — destinatários: %s | horários: %s',
            ...
            while not _stop_event.is_set():
                ...
                if proximo is None:
                    proximo = datetime.now() + timedelta(hours=12)
                logger.info('[notificador_padioleiro] Próximo envio: %s', proximo.strftime('%d/%m/%Y %H:%M'))
